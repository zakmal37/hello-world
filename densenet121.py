...
def load_data(data_directory): #function for calling of images with labels
    ...
    for d in directories:
        ...
        file_names = [os.path.join(label_directory, f)
                      ...
                      if f.endswith(".tif")]#change the format as .jpg,.ppm or anything else in which data is formatted
        for f in file_names:
            images.append(skimage.data.imread(f))
            labels.append(int(d))
    return images, labels

ROOT_PATH = "D:/DataSet/OCT Images (DUKE)" #root directory of images
train_data_directory = os.path.join(ROOT_PATH, "Folder_wise/Train") #main directory of images in which train and tests images are present
test_data_directory = os.path.join(ROOT_PATH, "Folder_wise/Test")

train, train_labels = load_data(train_data_directory) #callig of function
...
score,accuracy = model.evaluate(np.array(images28_test), np.array(test_labels), verbose=1)
